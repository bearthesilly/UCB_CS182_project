...
TASK_A_ADAPTER_PATH = os.path.join(RESULTS_DIR, "math_adapter_llama_fp32_alpha_{}_")
...
PER_DEVICE_BS = 8
GRAD_ACC_STEPS = 4 # (有效批量大小仍然是 8 * 4 = 32)
...
def main(alpha):
    ...
    CURRENT_A_ADAPTER_PATH = TASK_A_ADAPTER_PATH.format(alpha)
    seq_trainer_a.train()

    print(f"--- Phase 1 (MATH) training complete. Saving adapter to {CURRENT_A_ADAPTER_PATH} ---")
    seq_model.save_pretrained(CURRENT_A_ADAPTER_PATH)
    print("Adapter saved.")
    ...
    history_filename = os.path.join(RESULTS_DIR, f"forgetting_history_MATH_to_HotpotQA_fp32_alpha_{alpha}.json")
    try:
    ...
    plot_filename = os.path.join(RESULTS_DIR, f"sequential_forgetting_curve_MATH_to_HotpotQA_fp32_for_alpha_{alpha}.png")
...
if __name__ == "__main__":
    ...
    for i, history in enumerate(histories):
        ...
        if not history:
            continue # skip   
        plt.plot(history["steps"], history["hotpot_loss"], 'o-', label=f"B(HotpotQA) alpha={cur_alpha}", color="blue")
        plt.plot(history["steps"], history["math_loss"], 'o-', label=f"A (MATH) alpha={cur_alpha}", color="red")
        # plt.plot(history["steps"], history["hotpot_loss"], 'o-', label="Task B (HotpotQA) Loss ", color="blue")
        # plt.plot(history["steps"], history["math_loss"], 'o-', label="Task A (MATH) Loss", color="red")
